...
public class OrdenJpaController implements Serializable {
    ...
    public void create(Orden orden) throws PreexistingEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            Reserva reservaCodigoReserva = orden.getReservaCodigoReserva();
            if (reservaCodigoReserva != null) {
                reservaCodigoReserva = em.getReference(reservaCodigoReserva.getClass(), reservaCodigoReserva.getCodigoReserva());
                orden.setReservaCodigoReserva(reservaCodigoReserva);
            }
            em.persist(orden);
            if (menuCodigoMenu != null) {
            ...
            if (reservaCodigoReserva != null) {
                reservaCodigoReserva.getOrdenCollection().add(orden);
                reservaCodigoReserva = em.merge(reservaCodigoReserva);
            }
            utx.commit();
        } catch (Exception ex) {
            try {
    ...
    public void edit(Orden orden) throws NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            Reserva reservaCodigoReservaOld = persistentOrden.getReservaCodigoReserva();
            Reserva reservaCodigoReservaNew = orden.getReservaCodigoReserva();
            ...
            if (reservaCodigoReservaNew != null) {
                reservaCodigoReservaNew = em.getReference(reservaCodigoReservaNew.getClass(), reservaCodigoReservaNew.getCodigoReserva());
                orden.setReservaCodigoReserva(reservaCodigoReservaNew);
            }
            ...
            if (reservaCodigoReservaOld != null && !reservaCodigoReservaOld.equals(reservaCodigoReservaNew)) {
                reservaCodigoReservaOld.getOrdenCollection().remove(orden);
                reservaCodigoReservaOld = em.merge(reservaCodigoReservaOld);
            }
            if (reservaCodigoReservaNew != null && !reservaCodigoReservaNew.equals(reservaCodigoReservaOld)) {
                reservaCodigoReservaNew.getOrdenCollection().add(orden);
                reservaCodigoReservaNew = em.merge(reservaCodigoReservaNew);
            }
            utx.commit();
        } catch (Exception ex) {
            try {
    ...
    public void destroy(Integer id) throws NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            Reserva reservaCodigoReserva = orden.getReservaCodigoReserva();
            if (reservaCodigoReserva != null) {
                reservaCodigoReserva.getOrdenCollection().remove(orden);
                reservaCodigoReserva = em.merge(reservaCodigoReserva);
            }
            em.remove(orden);
            utx.commit();
